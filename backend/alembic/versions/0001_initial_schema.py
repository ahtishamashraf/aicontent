...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('queued', 'running', 'completed', 'failed', name='analysis_status'), nullable=False),
    ...
    sa.Column('reliability', sa.Enum('insufficient', 'low', 'moderate', 'normal', name='reliability_level'), nullable=True),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.CheckConstraint('(user_id IS NOT NULL) OR (guest_token_hash IS NOT NULL)', name=op.f('ck_analyses_owner_or_guest_required')),
    sa.CheckConstraint('public_score IS NULL OR (public_score >= 0 AND public_score <= 100)', name=op.f('ck_analyses_public_score_range')),
    ...
    op.create_index(op.f('ix_analyses_guest_token_hash'), 'analyses', ['guest_token_hash'], unique=True)
    ...
    sa.Column('encrypted_text', sa.LargeBinary(), nullable=True),
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.ForeignKeyConstraint(['analysis_id'], ['analyses.id'], name=op.f('fk_analysis_segments_analysis_id_analyses'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_analysis_segments')),
    sa.UniqueConstraint('analysis_id', 'index', name='uq_segment_order')
    )
    op.create_index(op.f('ix_analysis_segments_analysis_id'), 'analysis_segments', ['analysis_id'], unique=False)
